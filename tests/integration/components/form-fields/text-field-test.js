...
moduleForComponent(
  ...
  'Integration | Component | form fields/text field',
  {
    integration: true,
...
test('A custom update action can be passed', function(assert) {
  assert.expect(1);
  this.on('update', (object, propertyPath, value) =>
    assert.equal(value, 'Mark')
  );
  this.render(
    hbs`{{form-fields/text-field propertyName object=object update=(action 'update')}}`
  );
  this.$('input')
    .val('Mark')
    .trigger('change');
